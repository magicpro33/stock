...
def _try_cascade_panel():
    """Use Money Weather's already-cached dump panel when this tab is
    running inside that app — avoids a second 20MB gzip parse."""
    try:
    ...
    for col in ("Piotroski", "ROIC", "OE_Yield", "ShortPctFloat",
        ...
        if col in fa:
            fund[col] = fa[col]
    P = dict(
        O=panel["o"].astype(np.float32, copy=False),
        H=panel["h"].astype(np.float32, copy=False),
        L=panel["l"].astype(np.float32, copy=False),
        C=panel["c"].astype(np.float32, copy=False),
...
def _live(n: int = 80):
    try:
        P = _try_cascade_panel()
        if P is not None:
            return score_from_panel(P, n=n)
    except Exception:
        pass
    data = _load_dump()
...
def _backtest() -> dict | None:
    path = os.path.join(LOCAL_DIR, BACKTEST_NAME)
...
def render_storm_watch_tab():
    st.subheader("⚡ Storm Watch")
    st.caption(
        "Finds names coiling at range lows after a short-term shakeout — "
        "the setup that actually preceded upside in the held-out half of this dump. "
        "Classic breakouts, MACD crosses, and 63-day momentum all flipped negative "
        "out of sample. Research tool, not investment advice."
    )

    go = st.button("Scan shakeout coils", type="primary") or st.session_state.get("_sw_ran")
    if not go:
    ...
    with fc1:
        min_sh = st.slider("Min shakeout (0–5)", 0, 5, 3)
    with fc2:
    ...
    st.download_button(
        "Download Storm Watch CSV",
        view.head(n_show).to_csv(index=False).encode(),
        file_name=f"storm_watch_{as_of}.csv",
        mime="text/csv",
    )
